...
const useStyles = makeStyles(theme => ({
  ...
  function createData(name, mentor, date, time, status,feedback) {
  return { name, mentor, date, time, status,feedback };
}

  const rows = [
  createData('user.png','Chukudi Titus' ,'24/05/2019', '3pm', 'Succesful', 'Give a feedback'),
  ...
  return (
    <DashBoard name='FeedBack'>
    <Paper className={classes.root}>
      <p style={{color:'blue',textAlign:'center'}}>Feedback goes back to admin</p>
          ...
          {rows.map(row => (
            <TableRow key={row.name}>
              ...
              <TableCell align="right"> <Button color="primary"
               ...
               className={classes.button}>{row.feedback}
               </Button></TableCell>
            </TableRow>
          ))}
        </TableBody>
